...
jQuery(function () {
    ...
    jQuery('#save_settings_first_page').on('click', function () {
        ...
        if(jQuery('#elex_default_google_category').val()) {
    ...
    jQuery('#elex_gpf_exclude_sold_quantity').change(function () {
      ...
      jQuery('#save_settings_cat_map, #save_settings_cat_map_product').on('click', function () {
        ...
        if( jQuery(this).attr('id') != 'save_settings_cat_map_product') {
            jQuery('#elex_cat_table').find('tbody').find('.check-column input:checkbox').each(function() {
            var temp_check_cat_checkbox = false;
            var temp_check_google_cats  = false;
            if(jQuery(this).prop("checked") == true) {
                selected_prod_cats.push(jQuery(this).val());
                temp_check_cat_checkbox = true;
            }
            if(jQuery("#elex_google_cats_"+jQuery(this).val()).val()) {
                selected_google_cats.push(jQuery("#elex_google_cats_"+jQuery(this).val()).val());
                temp_check_google_cats  = true;
            }
            if( temp_check_cat_checkbox != temp_check_google_cats ) {
                is_google_cats  = false;
                if(selected_prod_cats.length > 0) {
                    return false;
                }
            }
            });
            ...
            else if( ! is_google_cats ) {
            var map_cats = confirm("Google category is not mapped. We recommend you to map Google Category to increase the possibility of getting the feed approved from Google. Do you want to continue without mapping Google Category?");
            if (!map_cats == true) {
                jQuery(".elex-gpf-loader").css("display", "none");
                return;
            }
