...
if ( ! function_exists( 'elex_gpf_activate_basic_plugin' ) ) {
	function elex_gpf_activate_basic_plugin() {
		if ( is_plugin_active( 'elex-product-feed/elex-product-feed.php' ) ) {
			...
			wp_die( esc_html_e( 'Oops! You tried installing the Basic version without deactivating and deleting the Premium version. Kindly deactivate and delete ELEX Product Feed and then try again', 'elex-product-feed' ), '', array( 'back_link' => 1 ) );
		}
	}
}
