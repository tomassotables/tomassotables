...
class Elex_Gpf_Ajax_Call {
	...
	public function elex_gpf_generate_feed() {
		...
		$feed_report = $this->elex_gpf_create_project( $file_title, $project_desc, $sale_country, $sel_google_cats, $product_ids, $prod_attr, $google_attr, $exclude_ids, $autoset_identifier_exists, $condition, $prepend_attr, $append_attr, $feed_file_type, $currency_conversion, $featured, $stock_check, $stock_quantity, $prod_sold_check, $sold_quantity, $prod_vendors, $last_chunk, $chunk, $more_than_hundred, $is_edit_project, $file_to_edit );
		$start_filter = array();
	...
	public function elex_gpf_create_project( $project_title, $project_desc, $sale_country, $sel_google_cats, $product_ids, $prod_attr, $google_attr, $exclude_ids, $autoset_identifier_exists, $condition, $prepend_attr, $append_attr, $mime_type, $currency_conversion, $featured, $stock_check, $stock_quantity, $prod_sold_check, $sold_quantity, $prod_vendor, $last_chunk = false, $chunk = 0, $more_than_hundred = false, $is_edit_project = '', $file_to_edit_or_refresh = '' ) {
		$seperated_value = ',';
		$file_mime_type  = '.csv';
		...
		foreach ( $product_ids as $key => $val ) {
			...
			foreach ( $val as $ids ) {
				...
				if ( ( ! is_array( $exclude_ids ) || ! in_array( $ids, $exclude_ids ) ) && $vendor_check ) {
					...
					if ( $product->is_type( 'simple' ) && $this->elex_gpf_check_stock_and_sold_quantity( $product, $stock_check, $stock_quantity, $prod_sold_check, $sold_quantity ) && $is_featured ) {
	...
	public function elex_gpf_check_stock_and_sold_quantity( $product, $stock_check, $stock_quantity, $prod_sold_check, $sold_quantity ) {
		if ( ( ! $stock_check && ! $prod_sold_check ) || ( '' == $stock_quantity && '' == $sold_quantity ) ) {
			return true;
		}
		$product_quantity = 0;
	...
	public function elex_gpf_manage_feed_edit_file() {
		...
		$prefill_values['default_category_chosen'] = '';
	...
	public function elex_gpf_get_custom_meta_keys() {
		global $wpdb;
		$sql = 'SELECT  meta.meta_key  FROM ' . $wpdb->prefix . 'postmeta AS meta, ' . $wpdb->prefix . 'posts' . " AS posts WHERE meta.post_id = posts.id AND posts.post_type LIKE '%product%' AND (meta.meta_key NOT LIKE '\_%' OR meta.meta_key='_product_attributes') GROUP BY meta.meta_key ORDER BY meta.meta_key ASC;";
		$data = $wpdb->get_results( ( $wpdb->prepare( '%1s', $sql ) ? stripslashes( $wpdb->prepare( '%1s', $sql ) ) : $wpdb->prepare( '%s', '' ) ), ARRAY_A );
		$temp_arr = array();
		$custom_metas = array();
		foreach ( $data as $key ) {
			$temp_arr = array();
		...
		if ( $setting_tab_fields && isset( $setting_tab_fields['custom_meta'] ) ) {
			foreach ( $setting_tab_fields['custom_meta'] as $key => $value ) {
				$custom_metas[ $value ] = array(
					'label' => $value,
					'type' => 'meta',
					'grp_type' => 'Meta Values',
				);
			}
		}
		return $custom_metas;
