...
class Elex_Manage_Feeds_Ajax_Function {
	...
	public function elex_gpf_manage_feed_refresh_file() {
		...
		$feed_report = $generate_feed_obj->elex_gpf_create_project( $project_title, $project_desc, $value['sale_country'], $value['sel_google_cats'], $product_ids, $value['prod_attr'], $value['google_attr'], $value['exclude_ids'], $autoset_identifier_exists, $condition, $prepend_attr, $append_attr, $value['feed_file_type'], $value['currency_conversion'], $value['featured'], $value['stock_check'], $value['stock_quantity'], $value['prod_sold_check'], $value['sold_quantity'], $value['prod_vendor'], false, 0, false, $is_edit_project, $file_to_refresh );
		$feed_reports = array();
