...
<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<ul class="shop_table cart woocommerce-cart-form__contents">
			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
				...
				<li class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					<div class="product-name" data-title="<?php esc_attr_e( 'Product', 'woocommerce' ); ?>">
						...
						<?php if ( ( $delivery = get_field( 'cart_delivery_time', $product_id ) ) && ! empty( $delivery ) ): ?>
							<div class="product-delivery"
							     data-title="<?php esc_attr_e( 'Delivery', 'woocommerce' ); ?>">
								<?php echo $delivery; ?>
							</div>
						<?php endif; ?>
					</div>
		...
		<li>
			<a href="#" class="wc-cart-toggle">
				<?php esc_html_e( 'Heb jij een kortingscode?', 'woocommerce' ); ?>
			</a>

			<div class="actions">
				<?php if ( wc_coupons_enabled() ) { ?>
					<div class="coupon">
						<label for="coupon_code"><?php esc_html_e( 'Heb jij een kortingscode?', 'woocommerce' ); ?></label>
				...
				<button type="submit" class="button js-update-cart" style="display:none;"> name="update_cart"
