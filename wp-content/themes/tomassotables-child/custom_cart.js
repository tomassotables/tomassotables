...
jQuery( function( $ ) {
	...
	jQuery(document).on('change',"#pa_afmetingen",function(){
		var variants = jQuery(".variations_form.cart").attr('data-product_variations');
	    var available_variants = JSON.parse(variants);
		for(var v=0;v < available_variants.length;v++){console.log(available_variants[v].variation_id+' == '+jQuery("input[name=variation_id]").val());
		    if(available_variants[v].variation_id == jQuery("input[name=variation_id]").val()){
		        if(available_variants[v].is_in_stock === true){
		            jQuery(".levertijd_stock").html("<strong>Gratis bezorging</strong>"+ jQuery("input[name=delivery_time]").val());
		        }else{
		            jQuery(".levertijd_stock").html("Uit voorraad");
		        }
		        break;
		    }
		}
...
jQuery('.beperkt_header_items').hover(function(){
    if(jQuery(this).children(".sub_menu_holder").length > 0){
		...
		if(jQuery(".woocommerce-Price-amount").length > 0){
		    jQuery(".woocommerce-Price-amount").each(function(){
		        ...
		        if (total_cart_text.substring(total_cart_text.length - 3) == ',00') {
		        	if(jQuery(this).children("bdi").length > 0){
		            	jQuery(this).children("bdi").html('<span class="woocommerce-Price-currencySymbol"></span>&nbsp;'+total_cart_text.substring(0,total_cart_text.length - 3)+',-');
		           }else{
		           		jQuery(this).html('<span class="woocommerce-Price-currencySymbol"></span>&nbsp;'+total_cart_text.substring(0,total_cart_text.length - 3)+',-');
		           }
		        } 
		        	...
		        	if(jQuery(this).children("bdi").length > 0){
		           		jQuery(this).children("bdi").html('<span class="woocommerce-Price-currencySymbol"></span>&nbsp;'+new_total_price);
		        	}else{
		        		jQuery(this).html('<span class="woocommerce-Price-currencySymbol"></span>&nbsp;'+new_total_price);
		        	}
		        }
		    });
		}
